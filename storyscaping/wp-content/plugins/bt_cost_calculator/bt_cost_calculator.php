...
class bt_cost_calculator {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$proxy = new Cost_Proxy( $admin_email, $email_client, $email_confirmation, $subject, $m_name, $m_email, $m_phone, $m_address, $m_message, $m_date, $m_time, $accent_color, $rec_secret_key, $css_class, $currency, $currency_after );
...
class Cost_Proxy {
	function __construct( $admin_email, $email_client, $email_confirmation, $subject, $m_name, $m_email, $m_phone, $m_address, $m_date, $m_time, $m_message, $accent_color, $rec_secret_key, $css_class, $currency, $currency_after ) {
			...
			(function( $ ) {
				...
				c.find( '.btContactSubmit' ).click(function() {
					...
					var data = {
						...
						'currency_after': '<?php echo $this->currency_after; ?>',						
						...
						'date_text': '<?php echo bt_cost_calculator::$date_text; ?>',
						'time_text': '<?php echo bt_cost_calculator::$time_text; ?>',
...
function bt_quote_map_sc() {
	...
	$bt_quote_params = array(
		...
		array( 'param_name' => 'time_start', 'type' => 'dropdown', 'heading' => __( 'Time Input Start', 'bt_plugin' ),
			'value' => $time_array
		),
		array( 'param_name' => 'time_end', 'type' => 'dropdown', 'heading' => __( 'Time Input End', 'bt_plugin' ),
			'value' => $time_array
		),
		...
		array( 'param_name' => 'show_booking', 'type' => 'checkbox', 'value' => array( 'Yes' => 'yes' ), 'heading' => __( 'Show Date/Time Inputs', 'bt_plugin' ) ),
		array( 'param_name' => 'date_text', 'type' => 'textfield', 'heading' => __( 'Date Input Title', 'bt_plugin' ), 'value' => __( 'Preferred Service Date', 'bt_plugin' ) ),
		array( 'param_name' => 'time_text', 'type' => 'textfield', 'heading' => __( 'Time Input Title', 'bt_plugin' ), 'value' => __( 'Preferred Service Time', 'bt_plugin' ) ),
	...
	if ( function_exists( 'bt_rc_map' ) ) {
		...
		bt_rc_map( 'bt_cost_calculator', array( 'name' => __( 'Cost Calculator', 'bt_plugin' ), 'description' => __( 'Cost calculator container', 'bt_plugin' ), 'container' => 'vertical', 'icon' => 'bt_bb_icon_bt_bb_cost_calculator', 'accept' => array( 'bt_cc_item' => true, 'bt_cc_multiply' => true, 'bt_cc_group' => true, 'bt_hr' => true, 'bt_header' => true, 'bt_text' => true, 'bt_bb_separator' => true, 'bt_bb_headline' => true, 'bt_bb_text' => true ), 'toggle' => true,
			'params' => $bt_quote_params
		));
		
		bt_rc_map( 'bt_cc_item', array( 'name' => __( 'Cost Calculator Item', 'bt_plugin' ), 'description' => __( 'Single cost calculator element', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_cost_calculator_item',
			'params' => $bt_cc_item_params
		));
		
		bt_rc_map( 'bt_cc_multiply', array( 'name' => __( 'Cost Calculator Multiply', 'bt_plugin' ), 'description' => __( 'Cost calculator multiply container', 'bt_plugin' ), 'icon' => 'bt_bb_icon_bt_bb_cost_calculator_item', 'container' => 'vertical', 'accept' => array( 'bt_cc_item' => true ), 'show_settings_on_create' => false,
