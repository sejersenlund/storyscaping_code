...
if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
	...
	if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
		...
		$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'craft-beer' );
		$subtitle .= '<span class = "btProductSKU">' . esc_html__( 'SKU:', 'craft-beer' ) . ' ' . $sku . '</span>'; 
...
if ( boldthemes_get_option( 'hide_headline' ) ) {
	echo boldthemes_get_heading_html( 
		array( 
			...
			'size' => apply_filters( 'boldthemes_product_headline_size', 'extralarge' ), 
			'dash' => $dash
		)  
	);
}